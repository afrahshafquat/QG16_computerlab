...
library(ggplot2)
library(gridExtra)
g1<-ggplot(data=data.frame(y=-log10(pval_mx[,1]), x=1:n_geno))+geom_point(aes(x=x,y=y))+labs(y='-log10(pvalue)',x='Markers', title='Manhattan Plot: Phenotype 1')+geom_hline(yintercept = -log10(0.05/n_geno), color='red')
g2<-ggplot(data=data.frame(y=-log10(pval_mx[,2]), x=1:n_geno))+geom_point(aes(x=x,y=y))+labs(y='-log10(pvalue)',x='Markers', title='Manhattan Plot: Phenotype 2')+geom_hline(yintercept = -log10(0.05/n_geno), color='red')
grid.arrange(g1,g2)
```
